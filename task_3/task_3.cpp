...
int main()
{
	...
	*/
	...
	const int row{ 6 }; // Количество строк (рядов).
	const int col{ 6 }; // Количество колонок (столбцов).
	...
	for (int i = 0; i < row / 2; i++) // Обрабатываем массив.
	{
		for (int j = 0; j < col; j++)
		{
			int temp{ arr[j][i] };
	...
	for (int i = 0; i < row; i++) // Выводим обработанный массив.
	{
		for (int j = 0; j < col; j++)
			cout << setw(4) << arr[i][j];
		cout << endl;
